...
## 11.1 Post Layout Transient analysis of CMOS Inverter(hierarchical approach) using magic and ngspice
...
3. New spice file is generated by magic. In the new spice file, input pulse, control statements are added. 

4. Next we, simulate it using ngspice. The following waveform is obtained as shown in fig 8
...
5. To measure rise, fall and propagation delay times, .meas statements were added in postlayout spice file. 

6. Timing parameters of CMOS inverter can be calculated from the graph shown in fig 9 as follows: 
...
5. The waveforms in fig 8 is similar to that of pre-layout simulation. However, there is a small delay in the waveform that might be caused due to parasitics in the layout.

## Obervations from Prelayout and Postlyout of CMOS inverter: 
