...
1. Next, post-layout simulation is performed for the function using Magic. 

2. First, the spice file for the function is imported in magic and all the parasitic in the crcuit are extracted which is shown by the screenshot below: 



3. New spice file is generated by magic. In the new spice file, input pulse, control statements are added

4. Next we, simulate it using ngspice. The following waveform is obtained.



5. The waveform is similar to that of pre-layout simulation. However, I observe small delay in the waveform that might be caused due to parasitics in the layout.
